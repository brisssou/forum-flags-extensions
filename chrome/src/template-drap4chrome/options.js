...
function initControls() {
	var site = browser.extension.getBackgroundPage().site;
	
	var refTimeLbl = document.getElementById("REFRESH_TIME_label");
	refTimeLbl.innerText = browser.i18n.getMessage("REFRESH_TIME_label");
	...
	refTimeInput.onchange = function(event) {
		...
		var site = browser.extension.getBackgroundPage().site;
		if (isNaN(refTime)) {
			document.getElementById("REFRESH_TIME.error").innerText = browser.i18n.getMessage("errorOnlyInt");
		} else if (refTime < site.minRefreshTime) {
			document.getElementById("REFRESH_TIME.error").innerText = browser.i18n.getMessage("errorMinRefreshTime", String(site.minRefreshTime));
		} else {
	...
	maxOpenAllLbl.innerText = browser.i18n.getMessage("MAX_OPEN_ALL_label");
	maxOpenAllLbl.title = browser.i18n.getMessage("MAX_OPEN_ALL_label_title");
	...
	maxOpenAllInput.onchange = function(event) {
		...
		if (isNaN(maxOpenAll)) {
			document.getElementById("MAX_OPEN_ALL.error").innerText = browser.i18n.getMessage("errorOnlyInt");
		} else {
	...
	bgColorLbl.innerText = browser.i18n.getMessage("BG_COLOR_label");
		...
		setDefaultColorA.innerText = browser.i18n.getMessage("setDefaultColor");
	...
	directLinkLbl.innerText = browser.i18n.getMessage("USE_DIRECT_LINK_label");
	directLinkLbl.title = browser.i18n.getMessage("USE_DIRECT_LINK_label_title");
	...
	openCatLbl.innerText = browser.i18n.getMessage("OPEN_CAT_label");
	openCatLbl.title = browser.i18n.getMessage("OPEN_CAT_label_title");
	...
	aniIconLbl.innerText = browser.i18n.getMessage("ANIMATED_ICON_label");
	...
	contextMenuLbl.innerText = browser.i18n.getMessage("USE_CONTEXT_MENU_label");
	contextMenuLbl.title = browser.i18n.getMessage("USE_CONTEXT_MENU_label_title");
	...
	newTabLbl.innerText = browser.i18n.getMessage("NEW_TAB_label");
	newTabLbl.title = browser.i18n.getMessage("NEW_TAB_label_title");
	...
	onlyFavsLbl.innerText = browser.i18n.getMessage("ONLY_FAVS_label");
	onlyFavsLbl.title = browser.i18n.getMessage("ONLY_FAVS_label_title");
	...
	debugOnLbl.innerText = browser.i18n.getMessage("DEBUG_ON_label");
	debugOnLbl.title = browser.i18n.getMessage("DEBUG_ON_label_title");
	...
	openToFrontLbl.innerText = browser.i18n.getMessage("OPEN_TO_FRONT_label");
	//openToFrontLbl.title = browser.i18n.getMessage("OPEN_TO_FRONT_label_title");
	...
	showCatLbl.innerText = browser.i18n.getMessage("SHOW_CAT_label");
	...
	getTopicsLbl.innerText = browser.i18n.getMessage("GET_TOPICS_label");
	...
	getModoLbl.innerText = browser.i18n.getMessage("MODO_label");
	...
	getAvatarLbl.innerText = browser.i18n.getMessage("AVATAR_label");
	...
	getMpsLbl.innerText = browser.i18n.getMessage("GET_MPS_label");
	...
	document.title = browser.i18n.getMessage("optionsPageTitle", site.name);
	
	document.getElementById('muted_label').innerText = browser.i18n.getMessage("muted_label", site.name);
	initMuted();
}
	...
	if (mutedTopicsValue) {
		...
		for (var i = 0; i < mutedTopics.length; i++) {
			...
			if (topic.indexOf('#') != -1) {
				...
				mutedTopicsInnerHtml += title + " <button id=\""+btn_id+"\">"+browser.i18n.getMessage("options_unmute")+"</button><br/>";
...
konami.code = function() {
	if (browser.extension.getBackgroundPage().site.avatarsUrl && browser.extension.getBackgroundPage().site.alertsUrl) {
